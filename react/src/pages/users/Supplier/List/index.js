...
import { useLocation, useNavigate } from "react-router-dom";
import { useForm } from "react-hook-form";
import { CardSupplierSkeleton } from "../../../../components/CardSupplier/CardSupplierSkeleton";
...
const List = () => {
  ...
  const handlePageChange = (page, event) => {
    ...
    setCurrentPage(page);
    navigate(
      `/supplier?${new URLSearchParams(watch()).toString()}&page=${page}`
    );
  };

  const handleSubmitForm = async (event) => {
    event.preventDefault();
    navigate(`/supplier?${new URLSearchParams(watch()).toString()}`);
  };
                  ...
                  {loading
                    ? Array.from({ length: 12 }).map((_, index) => (
                        <Col xs={12} md={6} lg={4} xl={3} key={index}>
                          <CardSupplierSkeleton></CardSupplierSkeleton>
                        </Col>
                      ))
                    : top12Supplier?.items?.map((supplier) => (
                        <Col xs={12} md={6} lg={4} xl={3} key={supplier.id}>
